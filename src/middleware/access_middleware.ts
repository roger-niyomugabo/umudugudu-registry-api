...
export const isChiefUser = async (req, res, next) => {
    try {
        req.user = await decodeToken(req);
        if (req.user.role !== 'village_chief') {
            return out(res, 403, 'You don\'t have access to do that action', null, 'FORBIDDEN');
        }
        return next();
    } catch (error) {
...
export const isChiefUserOrResident = async (req, res, next) => {
    try {
        req.user = await decodeToken(req);
        if (req.user.role !== 'village_chief' && req.user.role !== 'resident') {
            return out(res, 403, 'You don\'t have access to do that action', null, 'FORBIDDEN');
        }
        return next();
    } catch (error) {
...
export const isAdminOrChiefUser = async (req, res, next) => {
    try {
        req.user = await decodeToken(req);
        if (req.user.role !== 'admin' && req.user.role !== 'village_chief') {
            return out(res, 403, 'You don\'t have access to do that action', null, 'FORBIDDEN');
        }
        return next();
    } catch (error) {
